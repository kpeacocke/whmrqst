...
def _resolve_settlement_event(hero: Hero) -> dict | None:
    ...
    event_log = StepLog.objects.create(
        ...
        action_type="event",
        rng_seed=event_seed,
        dice_rolled=[{"die": "d6", "result": rng.d6(), "context": "event-roll"}],
...
def _resolve_catastrophic_event(party: Party) -> dict | None:
    ...
    log = StepLog.objects.create(
        ...
        party=party,
        step_type="settlement",
        action_type="catastrophic_event",
        rng_seed=event_seed,
        dice_rolled=[{"die": "d6", "result": rng.d6(), "context": "catastrophic-roll"}],
        effects_applied=effects,
        narrative=selected.definition.get("narrative", selected.name),
    )
...
def _apply_event_effects(
    ...
    for effect in effect_defs:
        ...
        if effect_type == "hero_health" and hero:
            ...
            applied["party_gold_delta"] += value
        elif effect_type == "party_morale" and target_party:
            target_party.morale += value
            applied["party_morale_delta"] += value

    if hero:
        hero.save(update_fields=["current_health", "updated_at"])
    if target_party:
        target_party.save(update_fields=["gold", "morale", "updated_at"])
    return applied
...
def _apply_settlement_rule_flag(hero: Hero, rule_flag: str | None) -> None:
    if not rule_flag:
        return
